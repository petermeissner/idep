...
#' function giving back length of dim1 or 0
dim1 <- function(x){
  ifelse(is.null(dim(x)[1]), 0, dim(x)[1])
}

#' function giving back length of dim2 or 0
dim2 <- function(x){
  ifelse(is.null(dim(x)[2]), 0, dim(x)[2])
}

#' seq_dim1
seq_dim1 <- function(x){
  return(seq_len(dim1(x)))
}

#' seq_dim2
seq_dim2 <- function(x){
  return(seq_len(dim2(x)))
}
