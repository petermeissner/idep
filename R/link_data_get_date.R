...
link_data_get_date <- function(RESULTS, flatten=F){
  worker <- function(ftext){
    dtext      <- ftext[ grep("date\\s+[iop][sdf]\\s+acceptance", 
                              ftext, ignore.case=T)  ]
      daccept  <- null_to_na(dmy(dtext[!is.na(dtext)], quiet=T))
      daccept  <- as.POSIXct(daccept, origin="1970-01-01",tz="UTC")
    
    dtext      <- ftext[ grep("date\\s+[iop][sdf]\\s+promulgation", 
                              ftext, ignore.case=T)  ]
      dpromul  <- null_to_na(dmy(dtext[!is.na(dtext)], quiet=T))
      dpromul <- as.POSIXct(dpromul, origin="1970-01-01",tz="UTC")
    
    dtext      <- ftext[ grep("date\\s+[iop][sdf]\\s+enactment", 
                              ftext, ignore.case=T)  ]
